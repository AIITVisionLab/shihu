...
- 当前临时回退为 Wi-Fi 单路径，后续如有线恢复再单独切回
- K230 当前 Wi-Fi 地址：`192.168.1.103`
- RK3568 固定地址：`172.18.8.19`
- 当前局域网源流地址：`rtsp://192.168.1.103:8554/test`
- `edgelink-gateway` 已重新打开 `video_source_switch`，只接受 `192.168.1.0/24` 来源作为视频源
- 若 K230 的 Wi-Fi 地址后续变化，RK3568 会根据 `/api/ai` 来源地址自动收敛到新的 Wi-Fi 地址
