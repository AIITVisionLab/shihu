...
import 'package:sickandflutter/features/video/widgets/video_intro_card.dart';
...
class VideoPage extends ConsumerWidget {
        ...
        child: ListView(
          ...
          children: streamListAsync.when(
            ...
            error: (error, stackTrace) => <Widget>[
              const VideoIntroCard(streams: <VideoStreamInfo>[]),
              const SizedBox(height: 18),
              VideoErrorCard(onRetry: () => _refresh(ref)),
            ],
            data: (streams) => <Widget>[
              VideoIntroCard(streams: streams),
              const SizedBox(height: 18),
              if (streams.isEmpty)
                VideoEmptyCard(onRetry: () => _refresh(ref))
              else
                ..._buildStreamCards(streams),
            ],
          ),
        ),
      ),
  ...
  List<Widget> _buildStreamCards(List<VideoStreamInfo> streams) {
    return <Widget>[
      VideoGallery(streams: streams),
