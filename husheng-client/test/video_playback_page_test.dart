...
void main() {
  testWidgets('VideoPlaybackPage shows embedded actions on web', (
    tester,
  ) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: VideoPlaybackPage(
          ...
          sourceLabel: '主画面',
          platformTypeOverride: PlatformType.web,
        ),
      ),
    );
    ...
    expect(find.text('当前平台暂不支持软件内查看'), findsNothing);
  });
  ...
  testWidgets(
    ...
    (tester) async {
        ...
        const MaterialApp(
          home: VideoPlaybackPage(
            ...
            initialUrl:
                'http://101.35.79.76:1984/stream.html?src=k230&mode=webrtc,mse',
            sourceLabel: '主画面',
            platformTypeOverride: PlatformType.ohos,
          ),
        ),
      );
      await tester.pumpAndSettle();
      ...
      expect(
        find.textContaining('Android、iOS、macOS、Windows、Linux 或 Web'),
        findsOneWidget,
      );
    },
  );
